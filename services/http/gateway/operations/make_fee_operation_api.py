...
class MakeFeeOperationGatewayAPI(OperationsGatewayAPI):
    def __init__(self, client: httpx.Client | None = None):
        super().__init__(client)
        self.PATH = "/make-fee-operation"
        self.MAKE_FEE_OPERATION_PATH_NAME = self.OPERATIONS_PATH_NAME + self.PATH
        self.MAKE_FEE_OPERATION_API = self.OPERATIONS_API + self.PATH
    ...
    def send_request(self, card_id: str, account_id: str):
        ...
        extensions = {"path_name": self.MAKE_FEE_OPERATION_PATH_NAME}

        response = self.CLIENT.post(
            self.MAKE_FEE_OPERATION_API,
