...
class OpenDebitCardAccountGatewayAPI(AccountsGatewayAPI):
    def __init__(self, client: httpx.Client | None = None):
        ...
        self.PATH = "/open-debit-card-account"
        self.OPEN_DEBIT_CARD_ACCOUNT_PATH_NAME = self.ACCOUNTS_PATH_NAME + self.PATH
        self.OPEN_DEBIT_CARD_ACCOUNT_API = self.ACCOUNTS_API + self.PATH
    ...
    def send_request(self, user_id: str):
        ...
        extensions = {"path_name": self.OPEN_DEBIT_CARD_ACCOUNT_PATH_NAME}
        response = self.CLIENT.post(
            self.OPEN_DEBIT_CARD_ACCOUNT_API, json=user_data, extensions=extensions
        )
